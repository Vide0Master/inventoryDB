...
const auth = (args) => {
    return new Promise((resolve) => {
        switch (args.type) {
            ...
            case 'mkeyAuth': {
                if (args.mkey == '') {
                    resolve({ result: 'error', message: 'Ключ не введено!' })
                    return
                }
                db.get(`SELECT * FROM users WHERE masterkey = "${args.mkey}"`, (err, row) => {
                    if (err) {
                        console.log(err)
                        resolve({ result: 'error', message: 'Невірний ключ!' })
                    } else if (row != undefined) {
                        const key = SK()
                        db.run('UPDATE users SET private_key = ? WHERE masterkey = ?', [key, args.mkey], function (err) {
                            if (err) {
                                cLog(`Спроба авторизації користувача ${login} вдала, але викликала помилку SQL запиту!\n${err}`, "e")
                                resolve({ result: 'error', message: 'Помилка запиту бази данних!' });
                                return
